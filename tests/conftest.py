"""pytest configuration for the Posten Bring test suite."""
import sys
...
def auto_enable_custom_integrations(enable_custom_integrations):
    """Make ``custom_components.posten_bring`` loadable from config-flow / setup tests."""
    yield
...
def reset_one_shot_warnings():
    ...
    assertion would depend on which test already triggered that value first.
    """
    from custom_components.posten_bring import parcels

    parcels._unmapped_statuses_logged.clear()
    parcels._unmapped_directions_logged.clear()
    parcels._undetermined_archived_logged.clear()
    yield
