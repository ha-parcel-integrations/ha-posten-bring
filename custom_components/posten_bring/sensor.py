"""Sensor platform for the Posten Bring parcel tracker integration."""
...
async def async_setup_entry(
    hass: HomeAssistant,
    entry: PostenBringConfigEntry,
    async_add_entities: AddEntitiesCallback,
) -> None:
    """Set up Posten Bring sensor entities from a config entry."""
    ...
    for parcel in coordinator.data or []:
        entities.append(
            PostenBringParcelSensor(coordinator, entry, parcel.get("barcode", ""))
        )
    entities.append(PostenBringNextDeliverySensor(coordinator, entry))
    entities.append(PostenBringDeliveredParcelsSensor(coordinator, entry))
    entities.append(PostenBringLastUpdateSensor(coordinator, entry))
    entities.append(PostenBringOutgoingParcelsSensor(coordinator, entry))
    entities.append(PostenBringOutgoingDeliveredParcelsSensor(coordinator, entry))
    entities.append(PostenBringAwaitingPickupSensor(coordinator, entry))

    async_add_entities(entities)


class PostenBringIncomingParcelsSensor(
    CoordinatorEntity[PostenBringCoordinator], SensorEntity
):
    ...
    def _handle_coordinator_update(self) -> None:
        ...
        if new_barcodes:
            self._async_add_entities(
                PostenBringParcelSensor(self.coordinator, self._entry, barcode)
...
class PostenBringParcelSensor(CoordinatorEntity[PostenBringCoordinator], SensorEntity):
    """Per-parcel sensor reporting the status of one tracked Posten Bring parcel."""
    ...
    def __init__(
        self, coordinator: PostenBringCoordinator, entry: ConfigEntry, barcode: str
    ) -> None:
        """Initialize the sensor."""
        super().__init__(coordinator)
        self._entry = entry
...
class PostenBringNextDeliverySensor(
    CoordinatorEntity[PostenBringCoordinator], SensorEntity
):
    ...
    def __init__(
        self, coordinator: PostenBringCoordinator, entry: ConfigEntry
    ) -> None:
        """Initialize the sensor."""
        super().__init__(coordinator)
...
class PostenBringDeliveredParcelsSensor(
    CoordinatorEntity[PostenBringCoordinator], SensorEntity
):
    """Recently delivered tracked Posten Bring parcels."""
...
class PostenBringLastUpdateSensor(
    CoordinatorEntity[PostenBringCoordinator], SensorEntity
):
    """Diagnostic sensor reporting when Posten Bring was last polled successfully."""
...
class PostenBringOutgoingParcelsSensor(
    CoordinatorEntity[PostenBringCoordinator], SensorEntity
):
    """Summary sensor: count of active outgoing (``direction: send``) parcels."""
...
class PostenBringOutgoingDeliveredParcelsSensor(
    CoordinatorEntity[PostenBringCoordinator], SensorEntity
):
    """Summary sensor: count of delivered outgoing (``direction: send``) parcels."""
...
class PostenBringAwaitingPickupSensor(
    CoordinatorEntity[PostenBringCoordinator], SensorEntity
):
    """Sensor reporting the number of parcels ready for collection.

    Posten Bring's ``collectable`` item status reaches
    ``ParcelStatus.AT_PICKUP_POINT`` from a real raw status, so per
    CONVENTIONS.md this carrier exposes the suite's standard
    ...
    def _pickup_parcels(self) -> list[dict]:
        return [
            parcel
            for parcel in (self.coordinator.data or [])
    ...
    def native_value(self) -> int:
        ...
        return len(self._pickup_parcels())

    @property
    def extra_state_attributes(self) -> dict[str, Any]:
        """Return the extra state attributes."""
        return {"parcels": self._pickup_parcels()}
