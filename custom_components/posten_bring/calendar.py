"""Calendar platform for the Posten Bring parcel tracker integration."""
...
async def async_setup_entry(
    ...
    """Set up the Posten Bring deliveries calendar from a config entry."""
    async_add_entities([PostenBringDeliveriesCalendar(entry.runtime_data.coordinator, entry)])


class PostenBringDeliveriesCalendar(CoordinatorEntity[PostenBringCoordinator], CalendarEntity):
    """A read-only calendar of expected Posten Bring deliveries.
    ...
    def __init__(self, coordinator: PostenBringCoordinator, entry: ConfigEntry) -> None:
    ...
    def _events(self) -> list[CalendarEvent]:
        ...
        for parcel in self.coordinator.data or []:
            ...
            summary = sender or (f"Parcel {barcode}" if barcode else "Posten Bring parcel")
            description_parts = [
